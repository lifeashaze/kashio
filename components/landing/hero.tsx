...
export function Hero() {
  ...
  return (
    <section className="relative overflow-hidden px-4 sm:px-6 pb-16 pt-16 sm:pb-20 sm:pt-20 md:pb-32 md:pt-32"  aria-label="Hero section">
        ...
        <div className="mx-auto max-w-4xl text-center">
          ...
          {!isLoading && (
            <div className="mt-8 sm:mt-10 flex flex-col items-center gap-3 sm:gap-4 sm:flex-row sm:justify-center px-4">
              <Button
                ...
                asChild
              >
                <Link href={session ? "/home" : "/signup"} className="cursor-pointer">
                  {session ? "Dashboard" : "Start for free"} <ArrowRight className="ml-2 h-4 w-4" aria-hidden="true" />
                </Link>
              </Button>
            </div>
          )}
          ...
          <div className="mt-6 sm:mt-8 flex flex-wrap items-center justify-center gap-4 sm:gap-6 text-xs sm:text-sm text-muted-foreground px-4">
            <div className="flex items-center gap-1.5 sm:gap-2">
              <svg className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-primary flex-shrink-0" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                <path
                  fillRule="evenodd"
      ...
      <div className="pointer-events-none absolute left-1/2 top-0 -z-10 h-[800px] w-[800px] -translate-x-1/2 rounded-full bg-gradient-to-br from-primary/30 via-chart-1/20 to-transparent blur-[120px]" aria-hidden="true" />
      <div className="pointer-events-none absolute bottom-0 right-0 -z-10 h-[600px] w-[600px] rounded-full bg-gradient-to-tl from-chart-2/20 to-transparent blur-[100px]" aria-hidden="true" />
    </section>
  );
}
